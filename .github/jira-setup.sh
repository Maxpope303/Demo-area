...
# 
# This script populates your Jira instance with a complete project structure
# for the WebSphere to Liberty modernization project.
#
# Usage:
#   1. Make executable: chmod +x jira-setup.sh
#   2. Run: ./jira-setup.sh
#   3. Follow prompts to enter your Jira credentials
#
# What it creates:
#   - Epic: WebSphere to Liberty Modernization
#   - User Stories (8 stories covering the full modernization)
#   - Tasks and subtasks
...
get_jira_config() {
    ...
    read -p "Jira Base URL (e.g., https://yoursite.atlassian.net): " JIRA_BASE_URL
    JIRA_BASE_URL=${JIRA_BASE_URL%/}  # Remove trailing slash
    ...
    read -p "Project Key (e.g., PHARM, MOD): " PROJECT_KEY
    PROJECT_KEY=$(echo "$PROJECT_KEY" | tr '[:lower:]' '[:upper:]')
    
    echo ""
    print_info "Configuration saved. Testing connection..."
}
...
get_project_id() {
    ...
    if [ -z "$PROJECT_ID" ]; then
        print_error "Project '$PROJECT_KEY' not found. Please create it first in Jira."
        exit 1
    fi
...
display_summary() {
    ...
    echo ""
    echo "Next steps:"
    echo "  1. Visit: $JIRA_BASE_URL/browse/$PROJECT_KEY"
...
main() {
    clear
    print_header "Jira Project Setup for Simple Pharmacy Modernization"
    
    print_info "This script will populate your Jira with a complete project structure."
    echo ""
